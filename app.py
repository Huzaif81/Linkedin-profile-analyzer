...
# 2. Permanent Backend API Configuration
try:
    # Pulls the key directly from your hidden cloud dashboard settings
    api_key = st.secrets["GEMINI_API_KEY"]
    genai.configure(api_key=api_key)
    llm_model = genai.GenerativeModel("gemini-2.5-flash")
except Exception:
    st.error("🔑 Developer Configuration Error: Master API Key is missing from the environment background secrets vault.")
    st.stop()
...
if uploaded_file and job_description.strip():
    if st.button("🚀 Execute Comprehensive Deep-Scan Analysis", type="primary"):
        ...
        try:
            # Highly structured comprehensive prompt
            advanced_prompt = f"""
